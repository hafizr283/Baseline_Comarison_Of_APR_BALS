...
// ── 2-GPU entity-split experiment (SYNC-2026-07-20d) ─────────────────────
// One host thread per GPU; each thread owns a full copy of every device
// structure and runs the unmodified per-batch pipeline restricted to its
// entity range. After each phase the updated factor halves are exchanged
// through pinned host memory (this box has no P2P — PHB topology). RMSE is
// computed as per-GPU partial sums over disjoint rating slices and combined
// at a barrier; both threads then take the identical convergence decision.
static pthread_barrier_t g_bar;
static double g_sq[2][2];               // [gpu][train|test] partial SSE
static float *g_Xex = nullptr, *g_Yex = nullptr;   // pinned exchange buffers
...
int main(int argc, char* argv[]) {
    ...
    // Weighted mode bakes λ·nnz into the gmem diag; solvers then add nothing.
    ...
    // ── 2-GPU split points ──
    // Per-entity work model t ≈ a·count (K³ solve) + b·nnz (LHS), a/b from
    // the measured K=96 fp16nr profile (183 ms / 480k users; 130 ms / 80.5M
    // nnz). Split at the 32-aligned (tile-row) index that balances t.
    int u_split = 0, i_split = 0;
    {
        const double wa = 0.381e-6, wb = 1.61e-9;
        double tot = wa * num_users + wb * nnz_train, acc = 0;
        for (int u = 0; u < num_users; u++) {
            acc += wa + wb * (h_user_offsets[u + 1] - h_user_offsets[u]);
            if (acc >= tot / 2) { u_split = (u + 31) & ~31; break; }
        }
        tot = wa * num_items + wb * nnz_train; acc = 0;
        for (int i = 0; i < num_items; i++) {
            acc += wa + wb * (h_item_offsets[i + 1] - h_item_offsets[i]);
            if (acc >= tot / 2) { i_split = (i + 31) & ~31; break; }
        }
        // argv[3] overrides the model-based user split (32-aligned) so the
        // balance point can be found empirically — the a/b model missed on
        // the first try (GPU1 straggled ~170 ms/iter at the exchange).
        if (argc > 3) u_split = (atoi(argv[3]) + 31) & ~31;
        printf("2-GPU split: users [0,%d)|[%d,%d)  items [0,%d)|[%d,%d)\n",
               u_split, u_split, num_users, i_split, i_split, num_items);
    }
    cudaHostAlloc(&g_Xex, sizeof(float) * (size_t)num_users * K, cudaHostAllocPortable);
    cudaHostAlloc(&g_Yex, sizeof(float) * (size_t)num_items * K, cudaHostAllocPortable);
    pthread_barrier_init(&g_bar, nullptr, 2);
    #define BAR() pthread_barrier_wait(&g_bar)

    auto worker = [&](int g) {
    cudaSetDevice(g);
    const int W_U0 = (g == 0) ? 0 : u_split, W_U1 = (g == 0) ? u_split : num_users;
    const int W_I0 = (g == 0) ? 0 : i_split, W_I1 = (g == 0) ? i_split : num_items;
    const long long tr_off = (g == 0) ? 0 : nnz_train / 2;
    const int       tr_cnt = (g == 0) ? (int)(nnz_train / 2) : nnz_train - (int)(nnz_train / 2);
    const long long te_off = (g == 0) ? 0 : nnz_test / 2;
    const int       te_cnt = (g == 0) ? (int)(nnz_test / 2) : nnz_test - (int)(nnz_test / 2);
    ...
    #define CHOL_MP_SOLVE(KK, NTH, CACHE) do { \
        ...
        if (chol_fresh) cholesky_solve_tiled_mp<KK, NTH, chol_store_t><<<bn, NTH, cholesky_tiled_smem_mp<KK, chol_store_t>(), sv>>>(lhs_b, rhs_b, d_chol_map, chol_nvec, chol_ntot, bn, solver_lambda, d_chol_fail, Lc_b_); \
        else { \
            cudaMemsetAsync(d_stale_cnt, 0, sizeof(int), sv); \
            cholesky_stale_refine<KK, NTH, chol_store_t><<<bn, NTH, cholesky_stale_smem<KK, chol_store_t>(), sv>>>(lhs_b, rhs_b, Lc_b_, bn, solver_lambda, d_chol_fail, d_stale_cnt, d_stale_list, d_stale_total, (float)CHOL_STALE_TAU); \
            cholesky_solve_tiled_mp<KK, NTH, chol_store_t><<<bn, NTH, cholesky_tiled_smem_mp<KK, chol_store_t>(), sv>>>(lhs_b, rhs_b, d_chol_map, chol_nvec, chol_ntot, bn, solver_lambda, d_chol_fail, Lc_b_, d_stale_list, d_stale_cnt); \
        } \
    } while (0)
#else
    ...
    auto run_training = [&](bool use_mixed, const char* label) -> TrainResult {
        ...
        for (int iter = 0; iter < max_iters; iter++) {
            ...
            for (int bstart = W_U0; bstart < W_U1; bstart += lhs_batch) {
                int bend = min(bstart + lhs_batch, W_U1);
                int bn   = bend - bstart;
                int bidx = (bstart - W_U0) / lhs_batch;
...
#if WEIGHTED_LAMBDA
                // diag += nnz_e * lambda (cumf_als parity); on sC after ALL of
                // this batch's LHS accumulation, before the sC->sS handoff.
                ...
                else if (K == 96) CHOL_MP_SOLVE(96, 128, d_Lcache_u);
#endif
#ifdef USE_CUSOLVER
            ...
            cudaEventRecord(t1);
            // Exchange X halves through pinned host memory (no P2P). The
            // D2H/H2D land in the t1-t2 gap, so the exchange cost shows up
            // in the (otherwise ~0) "User Cholesky" profile row.
            cudaDeviceSynchronize();
            cudaMemcpy(g_Xex + (size_t)W_U0 * K, d_X + (size_t)W_U0 * K,
                       sizeof(float) * (size_t)(W_U1 - W_U0) * K, cudaMemcpyDeviceToHost);
            BAR();
            {
                int oU0 = (g == 0) ? u_split : 0, oU1 = (g == 0) ? num_users : u_split;
                cudaMemcpy(d_X + (size_t)oU0 * K, g_Xex + (size_t)oU0 * K,
                           sizeof(float) * (size_t)(oU1 - oU0) * K, cudaMemcpyHostToDevice);
            }
            BAR();
            cudaEventRecord(t2);
            ...
            for (int bstart = W_I0; bstart < W_I1; bstart += lhs_batch) {
                int bend = min(bstart + lhs_batch, W_I1);
                int bn   = bend - bstart;
                int bidx = (bstart - W_I0) / lhs_batch;
                float* lhs_b = d_LHS_buf[bidx % nbuf];
                int prevb = bidx - nbuf;
                if (prevb >= 0 && prevb < MAX_PIPE_EVT)
            ...
            // Exchange Y halves (items are tiny: ~7 MB total at K=96).
            cudaDeviceSynchronize();
            cudaMemcpy(g_Yex + (size_t)W_I0 * K, d_Y + (size_t)W_I0 * K,
                       sizeof(float) * (size_t)(W_I1 - W_I0) * K, cudaMemcpyDeviceToHost);
            BAR();
            {
                int oI0 = (g == 0) ? i_split : 0, oI1 = (g == 0) ? num_items : i_split;
                cudaMemcpy(d_Y + (size_t)oI0 * K, g_Yex + (size_t)oI0 * K,
                           sizeof(float) * (size_t)(oI1 - oI0) * K, cudaMemcpyHostToDevice);
            }
            BAR();

            if ((iter+1)%5==0 || iter==max_iters-1) {
                cudaEventRecord(t6);
                // Per-GPU partial SSE over disjoint rating slices; combined
                // at the barrier. Both threads then evaluate the IDENTICAL
                // doubles, so the convergence decision is taken in lockstep
                // with no extra synchronization.
                cudaMemset(d_sq_err, 0, sizeof(double));
                int rb_tr = (tr_cnt + 255) / 256;
                compute_RMSE_kernel<<<rb_tr,256>>>(d_train_users+tr_off,d_train_items+tr_off,d_train_ratings+tr_off,tr_cnt,d_X,d_Y,K,d_sq_err);
                double h_sq_train;
                cudaMemcpy(&h_sq_train, d_sq_err, sizeof(double), cudaMemcpyDeviceToHost);
                cudaMemset(d_sq_err, 0, sizeof(double));
                int rb_te = (te_cnt + 255) / 256;
                compute_RMSE_kernel<<<rb_te,256>>>(d_test_users+te_off,d_test_items+te_off,d_test_ratings+te_off,te_cnt,d_X,d_Y,K,d_sq_err);
                double h_sq_test;
                ...
                rmse_calls++;
                g_sq[g][0] = h_sq_train; g_sq[g][1] = h_sq_test;
                BAR();
                float curr_train = sqrtf((float)((g_sq[0][0] + g_sq[1][0]) / nnz_train));
                float curr_test  = sqrtf((float)((g_sq[0][1] + g_sq[1][1]) / nnz_test));
                BAR();   // both read g_sq before anyone can overwrite it
                if (g == 0) printf("Iter %2d | Train RMSE: %.6f | Test RMSE: %.6f\n", iter+1, curr_train, curr_test);
                final_train_rmse = curr_train;
                final_test_rmse  = curr_test;
                if (fabsf(prev_rmse - curr_train) < tol) {
                    if (g == 0) printf("Converged at iteration %d (train delta=%.6f < tol=%.3f)\n",
        ...
        // Sequentialize the two GPUs' profile blocks (GPU0 first).
        for (int prt = 0; prt < 2; prt++) {
        BAR();
        if (g != prt) continue;
        printf("\n=== Profiling GPU%d [users %d..%d, items %d..%d]: %s (%d iters) ===\n",
               g, W_U0, W_U1, W_I0, W_I1, label, total_iters);
        printf("(LHS GFlops/s rows use FULL-dataset FLOPs vs this GPU's share of the work — read ms/iter only; User Cholesky row includes the X-halves exchange)\n");
        ...
        float tc2=total_u_compute+total_u_solve+total_i_compute+total_i_solve;
        for (int i=0; i<10; i++) cudaEventDestroy(ev[i]);
        return {tc2, wall_ms, final_train_rmse, final_test_rmse};
    };

    TrainResult apr = run_training(true, "APR-BALS (Mixed Precision, 2-GPU entity split)");

    if (g == 0) {
        printf("\n=== APR-BALS final (2-GPU) ===\n");
    ...
    // Each thread copies back only its owned factor rows (disjoint ranges).
    cudaMemcpy(h_X.data() + (size_t)W_U0 * K, d_X + (size_t)W_U0 * K,
               sizeof(float) * (size_t)(W_U1 - W_U0) * K, cudaMemcpyDeviceToHost);
    cudaMemcpy(h_Y.data() + (size_t)W_I0 * K, d_Y + (size_t)W_I0 * K,
               sizeof(float) * (size_t)(W_I1 - W_I0) * K, cudaMemcpyDeviceToHost);
...
#endif
    // The guard paths above return int, so the lambda's deduced return type
    // is int — falling off the end here would be UB (and did abort at exit).
    return 0;
    };   // worker lambda
    ...
    cudaFreeHost(g_Xex); cudaFreeHost(g_Yex);
    return 0;
}
