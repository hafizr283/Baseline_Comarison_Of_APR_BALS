...
// CUDA 9+ removed the maskless warp-shuffle intrinsics; this 2016 code (sm_35/50)
// predates that. Shim to the full-warp _sync form so it builds on sm_86.
#if defined(__CUDACC__) && (CUDART_VERSION >= 9000)
template <typename T> __device__ __forceinline__
T __shfl(T v, int s, int w = 32)         { return __shfl_sync(0xffffffffu, v, s, w); }
template <typename T> __device__ __forceinline__
T __shfl_down(T v, unsigned d, int w = 32){ return __shfl_down_sync(0xffffffffu, v, d, w); }
#endif
#include <curand.h>
...
void sgd_update_k128(Parameter para, mf_model *model, mf_problem *prob, float scale)
{
    ...
    if(prob->u_grid*prob->v_grid == 1)
    {
        ...
        clock_t start = clock();
        struct timespec _sgd_t0, _sgd_t1;         // wall clock for fair timing
        clock_gettime(CLOCK_MONOTONIC, &_sgd_t0);
        ...
        clock_gettime(CLOCK_MONOTONIC, &_sgd_t1);
        double _sgd_wall = (_sgd_t1.tv_sec - _sgd_t0.tv_sec)
                         + (_sgd_t1.tv_nsec - _sgd_t0.tv_nsec) / 1e9;
        double time_ela = (clock()-start)/double(CLOCKS_PER_SEC);
        printf("update_per_sec:%f\n", prob->nnz*para.num_iters/time_ela);
        printf("SGD_TRAIN_SECONDS: %.6f for %d iters (k=%d)\n",
               _sgd_wall, para.num_iters, para.k);
