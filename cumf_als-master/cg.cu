...
void updateXWithCGHost(float * A, float * x, float * b, const int batchSize, const int f, const float cgIter){
	//CUDA-12 port: launch the strided variant with 96 threads (a warp multiple),
	//the author's own alternative ("WARP shuffle seems requiring this").
	//blockDim=f (e.g. 100) leaves a partial warp whose shuffles are undefined on sm_70+.
