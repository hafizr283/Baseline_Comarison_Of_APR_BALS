...
/*
 * CUDA-12 port + APR-BALS-style benchmark driver (2026-07-11).
 * Compute kernels above are UNTOUCHED. Host-side changes only:
 *  - cusparseScsrmm2 (removed in CUDA 11) -> generic cusparseSpMM.
 *    thetaT (f x n col-major) is registered as an n x f ROW-major dense
 *    matrix: identical memory, identical product, no transpose needed.
 *  - device buffers are allocated/copied ONCE before the loop instead of
 *    per-iteration cudaMalloc+H2D (removes PCIe traffic from the timed
 *    region; strengthens this baseline, never weakens it).
 *  - iteration protocol matches justapr.cu: RMSE every 5 iterations,
 *    converge when train-RMSE delta < tol, cap at max_iters.
 *  - test-RMSE launch fixed: grid was (nnz_test-1)/256, dropping the tail
 *    ratings; now (nnz_test-1)/256 + 1.
 */
float doALS(const int* csrRowIndexHostPtr, const int* csrColIndexHostPtr, const float* csrValHostPtr,
		...
		const int max_iters, const float tol, const int X_BATCH, const int THETA_BATCH, const int DEVICEID)
{
	...
	//dimension: F*N
	cudacall(cudaMalloc((void** ) &thetaT, (size_t)f * n * sizeof(thetaT[0])));
	//dimension: M*F
	cudacall(cudaMalloc((void** ) &XT, (size_t)f * m * sizeof(XT[0])));
	cudacall(cudaMalloc((void** ) &ytheta, (size_t)f * m * sizeof(ytheta[0])));
	cudacall(cudaMalloc((void** ) &ythetaT, (size_t)f * m * sizeof(ythetaT[0])));
	cudacall(cudaMalloc((void** ) &yTX, (size_t)f * n * sizeof(yTX[0])));
	cudacall(cudaMalloc((void** ) &yTXT, (size_t)f * n * sizeof(yTXT[0])));
	cudacall(cudaMalloc((void** ) &d_errors, error_size * sizeof(d_errors[0])));
	//hermitian buffers, sized for the largest batch (the last batch carries the remainder)
	long maxXBatch     = (long)(m - (X_BATCH-1)*(m/X_BATCH));
	if (maxXBatch < m/X_BATCH) maxXBatch = m/X_BATCH;
	long maxThetaBatch = (long)(n - (THETA_BATCH-1)*(n/THETA_BATCH));
	if (maxThetaBatch < n/THETA_BATCH) maxThetaBatch = n/THETA_BATCH;
	cudacall(cudaMalloc((void** ) &tt, (size_t)f * f * maxXBatch * sizeof(float)));
	cudacall(cudaMalloc((void** ) &xx, (size_t)f * f * maxThetaBatch * sizeof(float)));
	...
	cudacall(cudaMemcpy(thetaT, thetaTHost, (size_t ) ((size_t)n * f * sizeof(thetaT[0])), cudaMemcpyHostToDevice));
	//CG needs XT
	cudacall(cudaMemcpy(XT, XTHost, (size_t ) ((size_t)m * f * sizeof(XT[0])), cudaMemcpyHostToDevice));
	...
	//CUDA-12: generic SpMM descriptors replacing cusparseScsrmm2.
	//R  (CSR, m x n): item-major train matrix.  R^T (CSR, n x m): user-major (the CSC arrays).
	//thetaT/XT are f-contiguous per entity == ROW-major (n x f)/(m x f) views.
	cusparseSpMatDescr_t matR, matRT;
	cusparseDnMatDescr_t dnTheta, dnXT, dnYtheta, dnYTX;
	cusparsecall(cusparseCreateCsr(&matR, m, n, nnz, csrRowIndex, csrColIndex, csrVal,
			CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I, CUSPARSE_INDEX_BASE_ZERO, CUDA_R_32F));
	cusparsecall(cusparseCreateCsr(&matRT, n, m, nnz, cscColIndex, cscRowIndex, cscVal,
			CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I, CUSPARSE_INDEX_BASE_ZERO, CUDA_R_32F));
	cusparsecall(cusparseCreateDnMat(&dnTheta, n, f, f, thetaT, CUDA_R_32F, CUSPARSE_ORDER_ROW));
	cusparsecall(cusparseCreateDnMat(&dnXT,    m, f, f, XT,     CUDA_R_32F, CUSPARSE_ORDER_ROW));
	cusparsecall(cusparseCreateDnMat(&dnYtheta, m, f, m, ytheta, CUDA_R_32F, CUSPARSE_ORDER_COL));
	cusparsecall(cusparseCreateDnMat(&dnYTX,    n, f, n, yTX,    CUDA_R_32F, CUSPARSE_ORDER_COL));
	const float alpha = 1.0f;
	const float beta = 0.0f;
	size_t bufsz1 = 0, bufsz2 = 0;
	void *spmmBuf1 = 0, *spmmBuf2 = 0;
	cusparsecall(cusparseSpMM_bufferSize(cushandle, CUSPARSE_OPERATION_NON_TRANSPOSE, CUSPARSE_OPERATION_NON_TRANSPOSE,
			&alpha, matR, dnTheta, &beta, dnYtheta, CUDA_R_32F, CUSPARSE_SPMM_ALG_DEFAULT, &bufsz1));
	cusparsecall(cusparseSpMM_bufferSize(cushandle, CUSPARSE_OPERATION_NON_TRANSPOSE, CUSPARSE_OPERATION_NON_TRANSPOSE,
			&alpha, matRT, dnXT, &beta, dnYTX, CUDA_R_32F, CUSPARSE_SPMM_ALG_DEFAULT, &bufsz2));
	if (bufsz1) cudacall(cudaMalloc(&spmmBuf1, bufsz1));
	if (bufsz2) cudacall(cudaMalloc(&spmmBuf2, bufsz2));

	using namespace std;
	printf("\n=== cuMF-ALS (HPDC16 baseline, FP32 + CG solver) ===\n");

	int block_dim = f/T10*(f/T10+1)/2;
	if (block_dim < f/2) block_dim = f/2;

	float prev_rmse = 1e9f;
	float final_train_rmse = 0, final_test_rmse = 0;
	double total_u_compute=0, total_u_solve=0, total_i_compute=0, total_i_solve=0, total_rmse_t=0;
	int total_iters=0, rmse_calls=0;
	float* h_err = (float*) malloc (sizeof(float));

	double wall_t0 = seconds();
	double t1;
	for(int iter = 0; iter < max_iters ; iter ++){
		total_iters++;
		//////////////////////////  update X (item factors)  //////////////////////////
		t1 = seconds();
		cusparsecall(cusparseSpMM(cushandle, CUSPARSE_OPERATION_NON_TRANSPOSE, CUSPARSE_OPERATION_NON_TRANSPOSE,
				&alpha, matR, dnTheta, &beta, dnYtheta, CUDA_R_32F, CUSPARSE_SPMM_ALG_DEFAULT, spmmBuf1));
		...
		cudaDeviceSynchronize();
		cudaCheckError();
		total_i_compute += (seconds() - t1) * 1000.0;
		...
		for(int batch_id = 0; batch_id< X_BATCH; batch_id ++){
			...
			total_i_solve += (seconds() - t1) * 1000.0;
		}

		//////////////////////////  update theta (user factors)  //////////////////////////
		t1 = seconds();
		cusparsecall(cusparseSpMM(cushandle, CUSPARSE_OPERATION_NON_TRANSPOSE, CUSPARSE_OPERATION_NON_TRANSPOSE,
				&alpha, matRT, dnXT, &beta, dnYTX, CUDA_R_32F, CUSPARSE_SPMM_ALG_DEFAULT, spmmBuf2));
		...
		cudaDeviceSynchronize();
		cudaCheckError();
		total_u_compute += (seconds() - t1) * 1000.0;
		...
		for(int batch_id = 0; batch_id< THETA_BATCH; batch_id ++){
			...
			t1 = seconds();
			cudacall( cudaMemset(xx, 0, (size_t)f*f*batch_size*sizeof(float)) );
			if(f == 100)
				get_hermitian100<<<batch_size, 64, SCAN_BATCH * f/2*sizeof(float2)>>>
			...
			total_u_solve += (seconds() - t1) * 1000.0;
		}

		//////////////////////////  RMSE check, APR-BALS cadence  //////////////////////////
		if ((iter+1)%5==0 || iter==max_iters-1) {
			t1 = seconds();
			cudacall( cudaMemset(d_errors, 0, error_size*sizeof(float)) );
			RMSE<<<(nnz-1)/256 + 1, 256>>>
					(csrVal, cooRowIndex, csrColIndex, thetaT, XT, d_errors, nnz, error_size, f);
			cudaDeviceSynchronize();
			cudaCheckError();
			cublascall( cublasSasum(handle, error_size, d_errors, 1, h_err) );
			float curr_train = sqrt((*h_err)/nnz);

			cudacall( cudaMemset(d_errors, 0, error_size*sizeof(float)) );
			RMSE<<<(nnz_test-1)/256 + 1, 256>>>(cooVal_test, cooRowIndex_test, cooColIndex_test, thetaT, XT,
					d_errors, nnz_test, error_size, f);
			cudaDeviceSynchronize();
			cudaCheckError();
			cublascall( cublasSasum(handle, error_size, d_errors, 1, h_err) );
			float curr_test = sqrt((*h_err)/nnz_test);
			total_rmse_t += (seconds() - t1) * 1000.0;
			rmse_calls++;
	...
	double wall_ms = (seconds() - wall_t0) * 1000.0;
	double compute_ms = total_u_compute + total_u_solve + total_i_compute + total_i_solve;

	//profiling report, same shape/formula as justapr.cu ("User"=theta side, "Item"=X side)
	double ums = total_u_compute/total_iters, ims = total_i_compute/total_iters;
	double fps = (double)nnz*(f*(f+1)/2.0+f)*2.0;
	double gft = fps*2.0*total_iters/1e9;
	printf("\n=== Profiling: cuMF-ALS (HPDC16 baseline, FP32 + CG solver) (%d iters) ===\n", total_iters);
	printf("User LHS+RHS: %7.2f ms total | %5.2f ms/iter | %6.1f GFlops/s\n", total_u_compute, ums, fps/(ums*1e-3)/1e9);
	printf("User CG solve:%7.2f ms total | %5.2f ms/iter\n", total_u_solve, total_u_solve/total_iters);
	printf("Item LHS+RHS: %7.2f ms total | %5.2f ms/iter | %6.1f GFlops/s\n", total_i_compute, ims, fps/(ims*1e-3)/1e9);
	printf("Item CG solve:%7.2f ms total | %5.2f ms/iter\n", total_i_solve, total_i_solve/total_iters);
	...
	printf("Total FLOPs: %.2f GFLOPs | Throughput: %.1f GFlops/s (compute: %.1f)\n",
			gft, gft/(wall_ms*1e-3), gft/(compute_ms*1e-3));
	printf("Training Complete in %.3f seconds!\n", wall_ms/1000.0);

	printf("\n=== cuMF-ALS final ===\n");
	printf("Train RMSE: %.6f | Test RMSE: %.6f | compute=%.2f ms | wall=%.2f ms\n",
			final_train_rmse, final_test_rmse, compute_ms, wall_ms);
	printf("Params: F=%d, lambda=%.4f (weighted ALS-WR: diag += nnz_u*lambda), CG_ITER=%d, SCAN_BATCH=%d, X_BATCH=%d, THETA_BATCH=%d\n",
			f, lambda, CG_ITER, SCAN_BATCH, X_BATCH, THETA_BATCH);

	//copy feature vectors back to host
	cudacall(cudaMemcpy(thetaTHost, thetaT, (size_t ) ((size_t)n * f * sizeof(thetaT[0])), cudaMemcpyDeviceToHost));
	cudacall(cudaMemcpy(XTHost, XT, (size_t ) ((size_t)m * f * sizeof(XT[0])), cudaMemcpyDeviceToHost));
	cusparseDestroySpMat(matR); cusparseDestroySpMat(matRT);
	cusparseDestroyDnMat(dnTheta); cusparseDestroyDnMat(dnXT);
	cusparseDestroyDnMat(dnYtheta); cusparseDestroyDnMat(dnYTX);
	if (spmmBuf1) cudaFree(spmmBuf1);
	if (spmmBuf2) cudaFree(spmmBuf2);
	cudacall(cudaFree(tt)); cudacall(cudaFree(xx));
	cudacall(cudaFree(thetaT)); cudacall(cudaFree(XT));
	cudacall(cudaFree(ytheta)); cudacall(cudaFree(ythetaT));
	cudacall(cudaFree(yTX)); cudacall(cudaFree(yTXT));
	cudacall(cudaFree(csrVal)); cudacall(cudaFree(csrColIndex)); cudacall(cudaFree(csrRowIndex));
	cudacall(cudaFree(cscVal)); cudacall(cudaFree(cscColIndex)); cudacall(cudaFree(cscRowIndex));
	cudacall(cudaFree(cooRowIndex));
	cudacall(cudaFree(cooRowIndex_test)); cudacall(cudaFree(cooColIndex_test)); cudacall(cudaFree(cooVal_test));
	cudacall(cudaFree(d_errors));
	free(h_err);
